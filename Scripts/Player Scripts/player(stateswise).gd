...
func _ready() -> void:
	...
	# Warn if the dedicated wall climb input isn't defined in the project InputMap.
	if not InputMap.has_action("wall_climb"):
		print("Warning: InputMap action 'wall_climb' not found. Add it in Project Settings -> Input Map to enable wall climbing via the intended button.")
...
func enter_state(state: PlayerState) -> void:
	match state:
		PlayerState.IDLE, PlayerState.WALK, PlayerState.SPRINT:
			current_speed = walking_speed if state == PlayerState.IDLE or state == PlayerState.WALK else sprinting_speed
			head.position.y = lerp(head.position.y, 0.0, 0.1)
			standing_collision_shape.disabled = false
			crouching_collision_shape.disabled = true
		PlayerState.CROUCH:
			current_speed = crouching_speed
			head.position.y = lerp(head.position.y, crouching_depth, 0.1)
		...
		PlayerState.WALL_CLIMB:
			...
			# No disable_front_raycasts() - rays stay enabled for reliable detection
...
func update_state(delta: float) -> void:
	...
	# State Update Loop
	match current_state:
		PlayerState.IDLE: update_idle_state(delta, input_dir)
		PlayerState.WALK: update_walk_state(delta, input_dir)
		PlayerState.SPRINT: update_sprint_state(delta, input_dir)
		PlayerState.CROUCH: update_crouch_state(delta, input_dir)
		PlayerState.SLIDE: update_slide_state(delta, input_dir)
		PlayerState.JUMP, PlayerState.FALL: update_air_state(delta, input_dir)
		PlayerState.WALL_SLIDE: update_wall_slide_state(delta, input_dir)
		PlayerState.WALL_CLIMB: update_wall_climb_state(delta, input_dir)
		PlayerState.WALL_JUMP: update_wall_jump_state(delta, input_dir)
...
func update_idle_state(delta: float, input_dir: Vector2) -> void:
	apply_gravity(delta)
	if not is_on_floor(): change_state(PlayerState.FALL)
	elif Input.is_action_pressed("crouch"): change_state(PlayerState.CROUCH)
	elif input_dir != Vector2.ZERO:
		if Input.is_action_pressed("sprint"): change_state(PlayerState.SPRINT)
		else: change_state(PlayerState.WALK)
...
func update_walk_state(delta: float, input_dir: Vector2) -> void:
	...
	elif Input.is_action_pressed("sprint") and input_dir != Vector2.ZERO: change_state(PlayerState.SPRINT)
	elif input_dir == Vector2.ZERO: change_state(PlayerState.IDLE)
...
func update_sprint_state(delta: float, input_dir: Vector2) -> void:
	...
	elif not Input.is_action_pressed("sprint"): change_state(PlayerState.WALK)
	elif input_dir == Vector2.ZERO: change_state(PlayerState.IDLE)
...
func update_crouch_state(delta: float, input_dir: Vector2) -> void:
	...
	if not is_on_floor(): change_state(PlayerState.FALL)
		...
		if Input.is_action_pressed("sprint"): change_state(PlayerState.SPRINT)
		elif input_dir != Vector2.ZERO: change_state(PlayerState.WALK)
		else: change_state(PlayerState.IDLE)

func update_slide_state(delta: float, _input_dir: Vector2) -> void:
	apply_gravity(delta)
	slide_timer -= delta
	if slide_timer < 0.0:
		slide_timer = 0.0
...
func update_air_state(delta: float, _input_dir: Vector2) -> void:
	apply_gravity(delta)
	if is_on_floor():
		change_state(PlayerState.IDLE)
		return
	# Climb entry: Front rays + wall_climb pressed
	var on_wall_rays = ray_cast_is_wall_check_F.is_colliding() and ray_cast_is_still_wall_check_F.is_colliding()
	if on_wall_rays and Input.is_action_pressed("wall_climb"):
		change_state(PlayerState.WALL_CLIMB)
		return
	# Fallback to wall slide
	if is_on_wall_only() and wall_slide_enabled and velocity.y < 0:
		change_state(PlayerState.WALL_SLIDE)

func update_wall_slide_state(delta: float, _input_dir: Vector2) -> void:
	velocity.y -= WALL_CLIMB_GRAVITY * delta
	if velocity.y < -MAX_WALL_SLIDE_SPEED:
		velocity.y = -MAX_WALL_SLIDE_SPEED
	# Stick to wall
	var wall_normal = get_wall_collision()
	if wall_normal != Vector3.ZERO:
		var press_speed = 2.0
		var toward_wall = -wall_normal * press_speed
		velocity.x = toward_wall.x
		velocity.z = toward_wall.z
	if is_on_floor(): change_state(PlayerState.IDLE)
	elif not is_on_wall_only(): change_state(PlayerState.FALL)
	elif Input.is_action_pressed("wall_climb"):
		var on_wall = ray_cast_is_wall_check_F.is_colliding() and ray_cast_is_still_wall_check_F.is_colliding()
		if on_wall: change_state(PlayerState.WALL_CLIMB)

func update_wall_climb_state(delta: float, _input_dir: Vector2) -> void:
	var on_wall = ray_cast_is_wall_check_F.is_colliding() and ray_cast_is_still_wall_check_F.is_colliding()
	...
	if Input.is_action_pressed("wall_climb"):
		...
		if on_wall and not on_floor:
			...
			if wait_time_1 <= 0: wait_time_1 = 0.5
		wait_time_1 -= delta
		if wait_time_1 <= 0: change_state(PlayerState.FALL)
	elif no_wall or on_floor or vertical_climb_timer.get_time_left() <= 0:
		change_state(PlayerState.FALL)
	else:
		change_state(PlayerState.FALL)

func update_wall_jump_state(delta: float, _input_dir: Vector2) -> void:
	apply_gravity(delta)
...
func update_wall_conditions() -> void:
	...
	can_wall_jump = not is_on_floor() and is_on_wall_only() and (not enable_double_jump or (JUMPS_DONE >= 1 or wall_jump_window_timer > 0))
...
func handle_jump_input(delta: float) -> void:
	if wall_jump_window_timer > 0: wall_jump_window_timer -= delta
	...
	match current_state:
		...
		PlayerState.CROUCH, PlayerState.SLIDE:
			velocity.y = -JUMP_VELOCITY
			JUMPS_DONE = 1
			if not ray_cast_crouch.is_colliding(): change_state(PlayerState.JUMP)
		PlayerState.WALL_CLIMB:
			var wall_is_ending = ray_cast_is_wall_check_F.is_colliding() and !ray_cast_is_still_wall_check_F.is_colliding()
			if wall_is_ending:
				velocity.y = WALL_CLIMB_SPEED * 1.5
...
func _apply_wall_jump_turn(delta: float) -> void:
	if not rotating_to_wall_jump: return
...
# Helpers
func get_wall_collision() -> Vector3:
